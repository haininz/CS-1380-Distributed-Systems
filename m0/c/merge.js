...
function createInvertedIndex(count, url) {
  return {count, url};
}
...
if (fs.existsSync(globalIndexPath)) {
  ...
  globalIndexContent.split('\n').forEach((line) => {
    if (line) {
      ...
      let gram = parts[0];
      let indices = parts[1].split(' ').reduce(
          (acc, currentValue, currentIndex, array) => {
            if (currentIndex % 2 === 0) { // Even index: URL
              ...
              acc.push(createInvertedIndex(count, currentValue));
            }
            return acc;
          }, []);
...
rl.on('line', (line) => {
  ...
  if (invertedIndexMap.has(gram)) {
    ...
    invertedIndexMap.get(gram).push(createInvertedIndex(count, url));
  } else {
    // invertedIndexMap.set(gram, [new InvertedIndex(count, url)]);
    invertedIndexMap.set(gram, [createInvertedIndex(count, url)]);
  }
});
...
const mergeIndices = () => {
  ...
  invertedIndexMap.forEach((values, key) => {
    ...
    console.log(`${key} |` + values.map(
        (value) => ` ${value.url} ${value.count}`).join(''));
  });
};
